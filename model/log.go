...
func (l *Log) FindAllPosts(db *gorm.DB) (*[]Log, error) {
	...
	err = db.Debug().Model(&Log{}).Where("date_request BETWEEN ? AND ? AND name_service = ?", preview, last, "preview_letter_simple").Order("date_request desc").Limit(10).Find(&posts).Error
